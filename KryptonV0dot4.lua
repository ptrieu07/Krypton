...
-- --- CUSTOM KEYBIND COMPONENT ---
local function addKeybind(tab, label, keyIndex, callback)
    local f = Instance.new("Frame", tabPanels[tab]); f.Size = UDim2.new(1, 0, 0, 30); f.BackgroundTransparency = 1
    local l = Instance.new("TextLabel", f); l.Text = label; l.Size = UDim2.new(0, 200, 1, 0); l.TextColor3 = Color3.fromRGB(220, 220, 220); l.Font = Enum.Font.Gotham; l.TextSize = 13; l.TextXAlignment = Enum.TextXAlignment.Left; l.BackgroundTransparency = 1
    
    local b = Instance.new("TextButton", f); b.Size = UDim2.new(0, 80, 0, 20); b.Position = UDim2.new(1, -85, 0, 5)
    b.Text = Config[keyIndex].Name
    ...
    UserInputService.InputBegan:Connect(function(input, gpe)
        if changing and input.UserInputType == Enum.UserInputType.Keyboard then
            changing = false
            Config[keyIndex] = input.KeyCode
            b.Text = input.KeyCode.Name
            b.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
            AddLog("SUCCESS", label .. " set to " .. input.KeyCode.Name)
...
-- --- CREATING TABS (ORDER: Player, Aim, Visual, Teleport, Server, Setting) ---
createTab("Player", 1)
createTab("Aim", 2)
createTab("Visual", 3)
...
-- --- TAB 6: SETTING ---
addKeybind("Setting", "Fly Keybind", "FlyKey")
addKeybind("Setting", "Noclip Keybind", "NoclipKey")
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Config.FlyKey then
        ...
        AddLog("SUCCESS", "Flight toggled " .. (Config.Fly and "ON" or "OFF") .. " via Keybind")
        if Config.Fly then 
            StartFly() 
        else 
            pcall(function() 
    ...
    -- Noclip Keybind Action
    if input.KeyCode == Config.NoclipKey then
        Config.Noclip = not Config.Noclip
        AddLog("SUCCESS", "Noclip toggled " .. (Config.Noclip and "ON" or "OFF") .. " via Keybind")
    end
end)
